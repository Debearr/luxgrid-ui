:root{
  --bg: {{theme.bg}}; --panel: {{theme.panel}}; --muted: {{theme.muted}};
  --text: {{theme.text}}; --brand: {{theme.brand}};
  --radius: {{theme.radius}}px; --pad: 20px; --max: {{theme.maxWidth}}px; --header: {{theme.headerHeight}}px;
}
*{box-sizing:border-box}
html{scroll-behavior:smooth}
body{
  margin:0; background: {{#if theme.gradient}}radial-gradient(1100px 600px at 70% -10%, #18223a 10%, transparent 60%),{{/if}} var(--bg);
  color:var(--text); font:16px/1.6 Inter, ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, 'Helvetica Neue', Arial;
}
...
/* Anchor offset fix (prevents header cut-off) */
