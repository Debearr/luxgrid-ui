    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          const fb = await fetch('/data/dashboard/market.json')
          if (!fb.ok) throw new Error('Fallback JSON missing')
          marketData = await fb.json()
        }
        setData(marketData)
        setLastUpdated(new Date((marketData as MarketData).timestamp).toLocaleString())
      } catch (error) {
